...
- `page_size > 0`
- 分页采用任意页分配（非连续），`PagedAllocator` 内部维护 `request_pages` 页表
- `has_contiguous_memory` / `max_contiguous_block` 仍返回连续空闲页信息，用于碎片度量
...
```bash
# 测试
cargo test                          # 全部 (31 unit + 7 doctest)
cargo test --lib                    # 仅库测试
...
| 测试类别 | 数量 | 说明                                             |
| -------- | ---- | ------------------------------------------------ |
| 单元测试 | 31   | 每模块独立测试，覆盖创建/分配/释放/合并/碎片计算/调度逻辑 |
...
5. ✅ `MemoryBlock` 仍仅包含 `id/start/size/request_id`；`PagedAllocator` 内部通过 `request_pages` 维护物理页映射（Phase 2 开始），可通过 `pages_for_request()` 查询
...
3. ✅ 逻辑页表 → 物理块映射（PagedAllocator 已支持非连续分配 + `pages_for_request()` 暴露页表）
...
- [ ] `cargo check` 通过
- [ ] `cargo test` 全部通过 (31 unit + 7 doctest)

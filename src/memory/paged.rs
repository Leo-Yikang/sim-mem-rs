...
//! 2. 每个请求分配整数个页
//! 3. 使用位图跟踪页的使用状态
//! 4. 任意页分配（非连续），每个 `MemoryBlock` 记录分配大小，`PagedAllocator` 内部维护物理页表
//!
//! ## 内部碎片
//!
//! 分页分配可能产生内部碎片：
...
//! ## 页表管理
//!
//! 每个分配的内存块记录其占用的物理页号列表，支持非连续分配。

use super::{Allocator, AllocatorStats, MemoryBlock};
...
impl PagedAllocator {
    ...
    pub fn new(total_memory: usize, page_size: usize) -> Self {
        ...
        assert!(
            total_memory.is_multiple_of(page_size),
            "总内存必须能被页大小整除"
        );

        let n_pages = total_memory / page_size;

        Self {
            total_memory,
            page_size,
            pages: vec![PageStatus::Free; n_pages],
    ...
    /// 查找连续空闲页（用于 `has_contiguous_memory` / `max_contiguous_block`）
    ///
    /// 返回第一个长度足够的连续空闲页序列的起始页号。
    fn find_contiguous_free_pages(&self, num_pages: usize) -> Option<usize> {
        if num_pages == 0 {
            return Some(0);
        }
        self.pages
            .windows(num_pages)
            .position(|window| window.iter().all(|&s| s == PageStatus::Free))
    }

    /// 查找任意 `num_pages` 个空闲页（非连续），用于实际分配。
    fn find_n_free_pages(&self, num_pages: usize) -> Option<Vec<usize>> {
        if num_pages == 0 {
            return Some(Vec::new());
        }
        let free_pages: Vec<usize> = self
            .pages
            .iter()
            .enumerate()
            .filter(|(_, &s)| s == PageStatus::Free)
            .map(|(i, _)| i)
            .collect();
        if free_pages.len() >= num_pages {
            Some(free_pages[..num_pages].to_vec())
        } else {
            None
        }
    }
    ...
    /// 查询指定请求占用的物理页号列表。
    ///
    /// 返回 `None` 表示该请求尚未分配内存或已被释放。
    pub fn pages_for_request(&self, request_id: usize) -> Option<&[usize]> {
        self.request_pages.get(&request_id).map(|v| v.as_slice())
    }
    ...
    fn calculate_internal_fragmentation(&self) -> f64 {
        ...
        let total_internal_fragmentation: usize = self
            .allocated_blocks
            .values()
            .map(|block| {
...
impl Allocator for PagedAllocator {
    fn allocate(&mut self, size: usize) -> Option<MemoryBlock> {
        ...
        if let Some(physical_pages) = self.find_n_free_pages(num_pages) {
            // 分配页（非连续）
            for &page in &physical_pages {
                self.pages[page] = PageStatus::Allocated;
            }
            ...
            let memory_block = MemoryBlock {
                id: block_id,
                start: physical_pages[0] * self.page_size,
                size,
                request_id: block_id,
            };
            ...
            self.request_pages.insert(block_id, physical_pages);
    ...
    fn has_contiguous_memory(&self, size: usize) -> bool {
        self.find_contiguous_free_pages(self.pages_needed(size))
            .is_some()
    }

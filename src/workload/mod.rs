...
///
/// 表示一个大模型推理或训练任务，同时携带：
/// - 旧版（Phase 0）字段：`lifetime` + `memory_size`，用于一次性分配 / 释放语义。
/// - 新增 LLM 字段（Phase 1+）：`prompt_tokens` + `output_tokens`，用于按 token 粒度
///   的连续批处理调度（Continuous Batching / Iteration-level Scheduling）。
///
/// # 字段语义（Phase 1 调度模式）
///
/// - `prompt_tokens`：Prefill 阶段一次性占用的 KV Cache 内存单位数。
/// - `output_tokens`：Decode 阶段需要生成的 token 数；在简化模型中每步生成 1 个 token，
///   增量占用 1 个内存单位，因此一个请求最终占用 `prompt_tokens + output_tokens`。
///
/// 旧版字段在调度模式下被忽略；在 Phase 0 直接执行模式下仍然生效，从而保持向后兼容。
#[derive(Debug, Clone)]
pub struct Request {
    /// 请求ID
    pub id: usize,
    ...
    /// 生命周期（占用内存的时间，仅 Phase 0 直接执行模式使用）
    pub lifetime: u64,
    /// 需要的内存大小（内存单位，仅 Phase 0 直接执行模式使用）
    pub memory_size: usize,
    /// Prefill 阶段 token 数（= prefill 时一次性分配的 KV Cache 单位数）
    pub prompt_tokens: usize,
    /// Decode 阶段需要生成的 token 数
...
impl WorkloadGenerator {
    ...
    pub fn next_request(&mut self, current_time: u64) -> Option<Request> {
        ...
        // 默认 LLM 模型：将 memory_size 视为 prompt_tokens；将 lifetime 的一半视为
        // output_tokens，使得 Phase 1 的调度模式能直接复用本生成器，且总占用近似
        // memory_size + lifetime/2 的合理量级。
        let prompt_tokens = memory_size;
        let output_tokens = (lifetime / 2).max(1) as usize;
    ...
    pub fn new(num_requests: usize, interval: u64, lifetime: u64, memory_size: usize) -> Self {
        let requests = (0..num_requests)
            .map(|i| Request {
                ...
                lifetime,
                memory_size,
                prompt_tokens: memory_size,
                output_tokens: (lifetime / 2).max(1) as usize,
            })
            .collect();
